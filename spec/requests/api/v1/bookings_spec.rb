...
RSpec.describe 'Bookings API', type: :request do
  ...
  let!(:client) { create(:user, :confirmed, role: 'client', phone: '+0000000003', first_name: 'Carl', last_name: 'Lee', email: 'cli@example.com', password: 'password123') }
...
RSpec.describe "Api::V1::Bookings", type: :request do
  ...
  describe "POST /api/v1/bookings (role access)" do
    it "forbids master from creating bookings" do
      ...
      token = JSON.parse(response.body)['token']
      post "/api/v1/bookings", params: { master_id: master.id, time_slot_id: 1, booking: { service_id: service.id, client_name: 'Xavier', client_email: 'x@example.com' } }, headers: { 'Authorization' => "Bearer #{token}" }
