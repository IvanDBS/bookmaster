class Api::V1::Admin::UsersController < Api::V1::Admin::BaseController
  ...
  def index
    ...
    per_page = params[:per_page].to_i; per_page = 20 if per_page <= 0 || per_page > 100
    total = users.count
  ...
  def destroy
    ...
    if @user.id == current_user.id
      return render_error(code: 'forbidden', message: 'Нельзя удалить собственную учетную запись', status: :forbidden)
    end
    if @user.destroy
  ...
  def user_params
    params.require(:user).permit(:role, :first_name, :last_name, :phone)
  end
end
