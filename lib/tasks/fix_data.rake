namespace :data do
  ...
  task health_check: :environment do
    ...
    if cancelled_with_slots > 0
      puts "  ⚠️  #{cancelled_with_slots} cancelled bookings still have slots"
    end
    
    # Слоты без даты
    slots_no_date = TimeSlot.where(date: nil).count
    if slots_no_date > 0
      puts "  ⚠️  #{slots_no_date} slots without date"
    end
    
    # Некорректные слоты
    invalid_slots = TimeSlot.where('end_time <= start_time').count
    if invalid_slots > 0
      puts "  ⚠️  #{invalid_slots} invalid slots (end_time <= start_time)"
    end
