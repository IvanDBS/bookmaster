<template>
  <div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
    ...
    <div class="max-w-7xl mx-auto px-6 py-6">
      ...
      <!-- My Masters Section -->
      <div v-if="currentStep < 4" id="masters" class="bg-white rounded-lg shadow-sm border border-gray-200 mb-8">
        ...
        <div class="p-6">
          ...
          <div v-else>
            ...
            <div class="bg-white rounded-xl shadow-lg border border-gray-100 p-6">
              ...
              <div class="space-y-3">
                <button v-for="service in selectedMasterForBooking.services" :key="service.id" 
        ...
        <div class="p-6 space-y-8">
          ...
          <div class="flex items-center justify-between">
            <div class="flex items-center space-x-2" :class="currentStep >= 1 ? 'text-lime-600' : 'text-gray-400'">
              <div class="w-6 h-6 rounded-full flex items-center justify-center border text-xs font-semibold" 
          ...
          <div v-if="currentStep === 1" class="space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              <button v-for="type in serviceTypes" :key="type" @click="selectServiceType(type)"
                      :class="['flex flex-col items-center justify-center rounded-xl border-2 p-6 text-center transition-all duration-300 transform hover:scale-105', selectedServiceType===type? 'border-lime-500 bg-gradient-to-br from-lime-50 to-lime-100 shadow-lg' : 'border-gray-200 hover:border-lime-300 hover:shadow-md']">
          ...
          <div v-if="currentStep === 2" class="space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              <button v-for="master in mastersForType" :key="master.id" @click="selectMaster(master)"
                      :class="['rounded-xl border-2 p-5 text-left transition-all duration-300 transform hover:scale-105', selectedMaster?.id===master.id? 'border-lime-500 bg-gradient-to-br from-lime-50 to-lime-100 shadow-lg' : 'border-gray-200 hover:border-lime-300 hover:shadow-md']">
                <div class="flex items-center space-x-4">
                  <div class="w-12 h-12 rounded-full bg-gradient-to-br from-lime-400 to-lime-600 flex items-center justify-center text-white font-bold shadow-md">
                    {{ master.user.first_name[0] }}
                  </div>
                  <div class="flex-1">
                    <div class="font-bold text-gray-900 text-lg">{{ master.user.first_name }} {{ master.user.last_name }}</div>
          ...
          <div v-if="currentStep === 3" class="space-y-5">
            ...
            <div class="flex justify-between">
              <button class="text-sm text-gray-600" @click="currentStep=2">← Назад к выбору мастера</button>
            </div>
          </div>
          ...
          <div v-if="currentStep === 4" class="space-y-5">
            ...
            <div class="flex justify-between">
              <button class="text-sm text-gray-600" @click="goBackToMasters">← Назад к выбору мастера</button>
            </div>
          </div>
          ...
          <div v-if="currentStep === 5" class="space-y-5">
            ...
            <div class="rounded-xl border border-gray-200 p-4">
              ...
              <div class="font-semibold text-gray-900">{{ formatBookingDate(selectedDate) }} • {{ formatBookingTime(selectedSlot.start_time) }}–{{ formatBookingTime(selectedSlot.end_time) }}</div>
            </div>
            <div class="flex items-center justify-between">
              <button class="text-sm text-gray-600" @click="currentStep=4">← Назад</button>
              <button @click="submitBooking" class="bg-gradient-to-r from-lime-500 to-lime-600 hover:from-lime-600 hover:to-lime-700 text-white font-bold px-8 py-3 rounded-lg transition-all duration-300 text-sm shadow-md hover:shadow-lg transform hover:scale-105">Подтвердить запись</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div id="bookings" class="bg-white rounded-xl shadow-lg border border-gray-100 mb-8">
        ...
        <div class="p-6">
          ...
          <div v-else class="space-y-4">
            <div v-for="booking in currentBookings" :key="booking.id" class="flex justify-between items-center p-6 bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl border border-gray-200 hover:shadow-md transition-all duration-300">
              <div class="flex-1">
                <div>
    ...
    if (response.ok) {
      ...
      for (const master of mastersMap.values()) {
        try {
          ...
          const servicesResponse = await fetch(`http://localhost:3000/api/v1/services?master_id=${master.id}`, {
            headers: { 'Authorization': `Bearer ${authStore.token}` }
          })
          if (servicesResponse.ok) {
            const services = await servicesResponse.json()
            master.services = services
            console.log(`Loaded ${services.length} services for master ${master.first_name}:`, services.map(s => s.name))
          } else {
            console.error(`Failed to load services for master ${master.first_name}:`, servicesResponse.status)
            master.services = []
          }
        } catch (error) {
