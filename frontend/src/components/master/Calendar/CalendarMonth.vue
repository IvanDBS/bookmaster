<template>
  <div>
    <div class="flex items-center justify-between mb-4">
      <button v-if="showPreviousButton" @click="$emit('previous-month')" class="p-2 hover:bg-gray-100 rounded">
        <svg class="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
        </svg>
      </button>
      <h4 class="text-lg font-semibold text-gray-900">{{ monthYear }}</h4>
      <button v-if="showNextButton" @click="$emit('next-month')" class="p-2 hover:bg-gray-100 rounded">
        <svg class="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
